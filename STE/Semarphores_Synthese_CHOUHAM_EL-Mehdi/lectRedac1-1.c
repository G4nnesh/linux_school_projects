...
/* ------------------------------------------------------------------------ */
circBuffer_t buf;
pthread_mutex_t mutexEcran; //Déclaration du mutex_Ecran
/* ------------------------------------------------------------------------ */
...
int main(int argc, char * argv[])
{
    ...
    CHECK_T(pthread_mutex_init(&mutexEcran, NULL), "pthread_mutex_in()"); //Initialisation du mutex
...
#if !defined(NOREADER)
    CHECK(pthread_create(&tidLecteur, NULL, (pf_t)lecteur, 0),
          "pthread_create(lecteur)");
#endif

#if !defined(NOWRITER)
    CHECK(pthread_create(&tidRedact, NULL, (pf_t)redacteur, 0),
          "pthread_create(redacteur)");
#endif

#if !defined(NOREADER)
    CHECK(pthread_join(tidLecteur, NULL), "pthread_join(lecteur)");
#endif

#if !defined(NOWRITER)
    CHECK(pthread_join(tidRedact, NULL), "pthread_join(redacteur)");
#endif

    CHECK_T(pthread_mutex_destroy(&mutexEcran),"pthread_mutex_destroy()") ;//Suppression du mutex
...
void putLine (const char *s)
{
    CHECK_T(pthread_mutex_lock(&mutexEcran), "pthread_mutex_lock()"); //Vérouillage pour utilisation
    ...
    CHECK_T(pthread_mutex_unlock(&mutexEcran), "pthread_mutex_lock()"); //Dévérouillage du mutex
}
/* ------------------------------------------------------------------------ */
